        ...
        for team in teams:
            ...
            for player in players:
                if player.get("player_id") == player_guid:  # If player is in this team
                    ...
                    kills = int(player.get("player_stats").get("Kills", 0))
                    assists = int(player.get("player_stats").get("Assists",0))
                    deaths = int(player.get("player_stats").get("Deaths",0))
                    headshots = int(player.get("player_stats").get("Headshot",0))
                    headshot_percentage = int(player.get("player_stats").get("Headshots %",0))
                    mvps = int(player.get("player_stats").get("MVPs",0))
                    triple_kills = int(player.get("player_stats").get("Triple Kills",0))
                    quadro_kills = int(player.get("player_stats").get("Quadro Kills",0))
                    penta_kills = int(player.get("player_stats").get("Penta Kills",0))
                    kd_ratio = round(float(player.get("player_stats").get("K/D Ratio",0)), 2)
                    kr_ratio = round(float(player.get("player_stats").get("K/R Ratio",0)), 2)
                    apr_ratio = round(float(player.get("player_stats").get("Assists",0)) / rounds, 2)
                    dpr_ratio = round((int(player.get("player_stats").get("Deaths",0)) / rounds), 2)
                    total_rounds = rounds
