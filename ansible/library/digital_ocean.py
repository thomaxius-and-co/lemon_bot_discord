...
options:
  ...
  monitoring:
    description:
      - Optional, Boolean, install monitoring agent for your droplet during creation.
    required: false
    default: "no"
    choices: [ "yes", "no" ]
  wait:
...
class Droplet(JsonfyMixIn):
    ...
    @classmethod
    def add(cls, name, size_id, image_id, region_id, ssh_key_ids=None, virtio=True, private_networking=False, backups_enabled=False, user_data=None, ipv6=False, monitoring=False):
        ...
        monitoring_lower = str(monitoring).lower()
        json = cls.manager.new_droplet(name, size_id, image_id, region_id,
            ...
            backups_enabled=backups_enabled_lower, user_data=user_data, ipv6=ipv6_lower, monitoring=monitoring_lower)
        droplet = cls(json)
        return droplet
